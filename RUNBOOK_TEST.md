# Runbook (Technical / Test) — Azure Resource Diff & Diagram Tool

**Use this runbook to deploy and test this tool on any machine** (this one, or a different one). It contains full setup/deployment commands, including creating a Function App and test resources for validating change-detection — intended for a test/dev environment, not to be shown as a manager-facing explanation (see the separate Manager Runbook for that).
...
## Part 0 — Tools and applications required (install these first on any new machine)

| Tool | Purpose | Download link |
|---|---|---|
| **Azure CLI** (`az`) | Manages Azure resources from the command line — used throughout this runbook | https://learn.microsoft.com/en-us/cli/azure/install-azure-cli |
| **Azure Functions Core Tools** (`func`) | Deploys the Python code to a Function App — separate from Azure CLI | Windows x64 MSI: https://go.microsoft.com/fwlink/?linkid=2174087 <br> Or via winget: `winget install Microsoft.Azure.FunctionsCoreTools` |
| **Python 3.11** | Runtime the tool's code is written in and tested against | https://www.python.org/downloads/release/python-3119/ (or latest 3.11.x from https://www.python.org/downloads/) |
| **Git for Windows** (includes Git Bash) | Provides the Bash-compatible terminal this runbook's commands assume | https://git-scm.com/download/win |
| **Visual Studio Code** (optional but recommended) | Code editor for viewing/editing the project files | https://code.visualstudio.com/download |
| **Microsoft Azure Architecture Icons** | Official icon artwork embedded in the generated diagrams | https://learn.microsoft.com/en-us/azure/architecture/icons/ (download the full ZIP, extract locally — see Part O) |

### Quick install commands (run once on a new machine, in Git Bash)
```bash
...
```
**Close and reopen your terminal after installing** so the new PATH entries take effect, then verify each:
```bash
az --version
func --version
python --version
...
### Also needed
- **An Azure subscription** with `Reader` access (or higher) — to query resources and, if deploying automation, to create a Function App
- **A storage account** you have write access to, with three containers already created (`snapshots`, `diffs`, `diagrams`) — see Part A, Step 1
...
| Container | Holds | Why kept separate |
|---|---|---|
| `snapshots` | The full daily resource inventory (large JSON, one file per day) | This is the raw data the diff logic depends on — kept isolated so it can have its own retention/access policy without affecting the human-readable outputs |
| `diffs` | The change report (`.json` + `.md`) — what was added/removed/modified | The "what changed" answer, in both machine- and human-readable form — smaller, more frequently read by people than `snapshots` |
| `diagrams` | The rendered SVG diagram + change table | The visual output most people actually want to look at — kept separate so it can be linked/shared/embedded without exposing the underlying raw snapshot data alongside it |

Splitting these also means each container's contents can be given a **different retention policy** if needed — for example, `diagrams` is currently limited to the 2 most recent files, while `snapshots` and `diffs` can be kept indefinitely for a full audit trail, without one policy interfering with the other. If everything sat in a single container, you couldn't apply different lifecycle rules to different file types.
...
### Icon coverage expanded to 44 resource types
After the case-sensitivity fix, coverage was expanded from the original 9 icons to **44 common Azure resource types** (compute, networking, storage, web/app service, databases, containers, integration, security, monitoring). `prepare_icons.sh` automates finding and copying these from a downloaded Microsoft icon pack — see Part O below. 41 were found via the script's automatic search terms; 3 (`front-door.svg`, `static-web-app.svg`, `action-group.svg`) needed manual `find`/`cp` since the pack's actual filenames didn't match the guessed search terms exactly. All 44 are now confirmed present and rendering correctly in production.

---

## Part O — Expanding icon coverage further (adding new resource types)

Use `prepare_icons.sh` (at the project root) to bulk-copy icons for the 44 resource types currently mapped in `ICON_FILES`:

```bash
cd /c/Projects/azure-resource-diff
chmod +x prepare_icons.sh
./prepare_icons.sh "/c/Users/hp/Downloads/Azure_Public_Service_Icons_V24/Azure_Public_Service_Icons"
```

It reports found (✓) vs. missing (✗) icons. Anything missing simply falls back to a colored glyph — never an error.

### If a search term doesn't match (as happened with 3 icons)
Manually search the pack for the correct filename:
```bash
ICONPACK="/c/Users/hp/Downloads/Azure_Public_Service_Icons_V24/Azure_Public_Service_Icons"
find "$ICONPACK" -iname "*<keyword>*.svg"
```
Then copy directly using the real filename found:
```bash
cp "<real path from find>" snapshot_function/icons/<expected-filename>.svg
```
(The expected filename for each resource type is the value side of the `ICON_FILES` dict in `snapshot_function/__init__.py`.)

### To add a brand-new resource type not yet in the mapping at all
See **Part K** for the full walkthrough (add to `TYPE_STYLE`, `ICON_FILES`, optionally `_extract_edges` for relationship arrows, then redeploy).

### After adding/changing any icons — always verify the folder is lowercase
```bash
ls snapshot_function/ | grep -i icons
```
Must show exactly `icons` (all lowercase) — see Part M for why this specifically matters (Windows/Linux case-sensitivity).

---

## Part P — What's automatic vs. what you'd only touch again
